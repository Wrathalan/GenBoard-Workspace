...
export function App() {
  ...
  const importImages = async () => {
    try {
      ...
      useWorkspace.getState().addAssets(assets, center());
    } catch (e) {
      fail(e);
    }
  };
  const addText = () => {
    ...
    s.change([
      ...s.board.items,
      { id, type: 'text', position: center(), width: 320, height: 180, data: { text: '' } },
    ]);
    s.select([id]);
  };
  const fitSelection = () => {
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if (ctrl && e.key.toLowerCase() === 's') {
        ...
        window.imagine
          .clipboardImage()
          .then((a) => a && useWorkspace.getState().addAssets([a], center()))
          .catch(fail);
  ...
  return (
    <main className="app-shell">
      ...
      <Canvas key={board?.id || 'empty'} hand={hand} inspect={setViewer} />
      {!project && (
      ...
      {project && (
        <div key={project.folder} style={{ display: right === 'generate' ? 'contents' : 'none' }}>
          <Generation referenceRequest={referenceRequest} close={() => setRight(null)} />
        </div>
      )}
                ...
                {images[0] && (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => {
                        setReferenceRequest({ id: images[0].data.assetId!, at: Date.now() });
                        setRight('generate');
                      }}
                    >
      ...
      {project && (
        <>
          ...
          <nav className="toolbar" aria-label="Canvas tools">
            ...
            <i />
            <button title="Import images" aria-label="Import images" onClick={importImages}>
              <ImagePlus size={18} />
            </button>
